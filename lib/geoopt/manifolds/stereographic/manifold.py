...
class PoincareBall(Stereographic):
    ...
    def add_time(self,x: torch.Tensor) -> torch.Tensor:

        assert self.c>0, 'curvature k must be positive'

        spatial_sq = torch.sum(x**2, dim=1, keepdim=True)
        time_dim = torch.sqrt(1.0/self.c + spatial_sq)
        lorentz_coords = torch.cat([time_dim, x], dim=1)
