...
from .hierarchy_helpers import hierarchy_pos, json2rels
...
def plot_hierarchy(tree, ax, show_idx):
    ...
    nx.draw(
        tree.G, ax=ax, pos=tree.pos, node_size=10, alpha=0, with_labels=False, width=0.5, edge_color='#f0f0f0',
        node_color='#f0f0f0', font_size=2,
    )
    nx.draw(
        tree.G, ax=ax, pos=tree.pos, node_size=5, edgelist=edges, nodelist=show_names, node_color='darkgrey',
        with_labels=True, width=0.5, edge_color='grey',
    )
    nx.draw_networkx_nodes(
